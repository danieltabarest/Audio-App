...
var LiderProyectoAutenticado = false;
...
function ConfigTableLugaresTrabajo() {
    var dtData = []; otLugaresTrabajo = $('#tblLugarDeTrabajoUPM').dataTable({
        ...
        "iDisplayLength": 10,
        "bAutoWidth": false,
        "aoColumns": [{ "sWidth": "25%" }, { "sWidth": "25%" }, { "sWidth": "25%" }, { "sWidth": "25%" }, { "sWidth": "25%" }, { "sWidth": "10%" }, { "sWidth": "10%" }],
        "oLanguage": {
            "sProcessing": "Procesando...",
            "sLengthMenu": "Mostrar _MENU_ registros",
            "sZeroRecords": "No se encontraron resultados",
...
function GetVisitas() {
    ...
    if ($("#txtProyectoId").val() !== "") {
        ...
        $.ajax({
            type: "POST",
            url: "/Visitas/GetVisitasProfesionalXProyectoInconformidad",
            data: params,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (data) {
                ...
                for (var x = 0; x < data.length; x++) {
                    ...
                    var Actions = ' <a href="#" onclick="ObtenerInformacionVisita(' + data[x].VisitaId + ',&quot;' + data[x].Codigo + '&quot;,&quot;' + data[x].UPM.Codigo + '&quot;,&quot;' + data[x].UPM.UPMId + '&quot;)"><i class="fa fa-eye fa-2x text-navy"></i></a>';
...
function ObtenerInformacionVisita(idVisita, codigoVisita, upmCodigo, upmId) {
    ...
    $("#txtCodigoVisita").val(codigoVisita);
    $("#txtUPMId").val(upmId);
    $("#txtVisitaId").val(idVisita);
        ...
        success: function (dataLtUPM) {
            ...
            if (dataLtUPM.length > 0) {
                ...
                for (var x = 0; x < dataLtUPM.length; x++) {
                    ...
                    var onclickViewUPM = "GetEncabezadoUPMLider('" + codigoVisita + "',true," + dataLtUPM[x].LugarTrabajoId + ")";
                    var onclickViewLT = "GetEncabezadoUPMLider('" + codigoVisita + "',false," + dataLtUPM[x].LugarTrabajoId + ")";

                    var ActionViewUPM = '<a href="#" onclick=' + onclickViewUPM + '><i class="fa fa-eye fa-2x text-navy"></i></a>';
                    var ActionViewLugarTrabajo = ' <a href="#" onclick=' + onclickViewLT + '><i class="fa fa-eye fa-2x text-navy"></i></a>';
                    ...
                    //    [dataLtUPM[x].Codigo, dataLtUPM[x].Nombre, NumLugar, dataLtUPM[x].TipoLugar, dataLtUPM[x].Etapa, ActionViewLugarTrabajo, ActionViewUPM]);
...
function GetEncabezadoUPMLider(CodigoVisita, esUPM, IdLugarTrabajo) {

    var urlServicio = "";
    var params = new Object();
    
    $("#txtLugarTrabajoId").val(IdLugarTrabajo);

    if (esUPM) {
        urlServicio = "/Visitas/GetEncabezadoUPMLider";
        params.CodigoVisita = CodigoVisita;
    }
    else {
        urlServicio = "/Visitas/GetEncabezadoLugarTrabajoLider";
        params.CodigoVisita = CodigoVisita;
        params.IdLugarTrabajo = IdLugarTrabajo;
    }

    var visita = CodigoVisita;
        ...
        success: function (dataEncabezados) {
            ...
            if (dataEncabezados.length > 0) {
                for (var x = 0; x < dataEncabezados.length; x++) {
                    ...
                    var onclickEncabezado = "ObtenerComponentesTagLider(" + dataEncabezados[x].AspectoId + ",'" + CodigoVisita + "'," + esUPM + "," + dataEncabezados[x].AspectoId + ",'" + nombreAspecto + "');";
...
function ObtenerComponentesTagLider(idAspecto, CodigoVisita, esUPM, idTag, nombreAspecto) {
    ...
    if (agregarComponentes) {
        ...
        $.ajax({
            type: "POST",
            url: "/Visitas/GetTitulosComponentesPorAspectoLider",
            data: params,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (dataComponentes) {
                ...
                if (dataComponentes.length > 0) {
                    ...
                    for (var x = 0; x < dataComponentes.length; x++) {
                        ...
                        htmlComponente += '<a onClick="ObtenerPreguntasRespuestasLider(\'' + CodigoVisita + '\',' + idAspecto + ',' + dataComponentes[x].ComponenteId + ',\'' + dataComponentes[x].Descripcion + '\',' + esUPM + ');" data-toggle="collapse" data-parent="#PanelComponentes-' + idAspecto + '" id="C' + dataComponentes[x].ComponenteId + '" href="#collapse' + dataComponentes[x].ComponenteId + '">' + dataComponentes[x].Orden + ' - ' + dataComponentes[x].Descripcion + '</a>';
...
function ObtenerPreguntasRespuestasLider(CodigoVisita, idaspecto, idcomponente, nombreAspecto, esUPM) {
    var params = new Object();
    var agregarPreguntasRespuestas = true;
    var reg = '';
    ...
    if (agregarPreguntasRespuestas) {
        ...
        $.ajax({
            type: "POST",
            url: "/Visitas/GetVariablesRespuestasPrimerNivelComponenteLider",
            data: params,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (datarespuestasPrimerNivel) {
                ...
                if (datarespuestasPrimerNivel.length > 0) {
                    ...
                    tablaPreguntasRespuestas += ' <tbody>';
                    for (var z = 0; z < datarespuestasPrimerNivel.length; z++) {
                        reg = datarespuestasPrimerNivel[z].VariableId + "|" + datarespuestasPrimerNivel[z].RespuestaUPMId + "|" + datarespuestasPrimerNivel[z].Pregunta + "|" + datarespuestasPrimerNivel[z].VisitaId;
                        if (datarespuestasPrimerNivel[z].Hijos > 0) {
                            tablaPreguntasRespuestas += '<tr id="TieneHijos-' + datarespuestasPrimerNivel[z].RespuestaUPMId + '">';
                            tablaPreguntasRespuestas += '<td><a href="#"><i id="iconExpand-' + datarespuestasPrimerNivel[z].RespuestaUPMId + '" onClick="AbrirRespuestasSegundoNivelLider(' + datarespuestasPrimerNivel[z].RespuestaUPMId + ',' + datarespuestasPrimerNivel[z].VariableId + ',' + esUPM + ',\'' + CodigoVisita + '\');" class="fa fa-plus text-navy"></i></a></td>';
                        } else {
                            tablaPreguntasRespuestas += '<tr>';
                            tablaPreguntasRespuestas += "<td>-</td>"
                        }
                        ...
                        if ((datarespuestasPrimerNivel[z].Hijos > 0)) {
                            ...
                            tablaPreguntasRespuestas += '<td align="center"><button type="button" class="btn btn-default" onclick="LoadFieldsInconformidad(' + "'" + reg + "'" + ')"><span class="glyphicon glyphicon-thumbs-down"></span></button></td>'
                        }
...
function LoadFieldsInconformidad(reg) {
    ...
    $("#txtPregunta").val(sreg[2]);

    if (LiderProyectoAutenticado) {
        $("#txtVisitaId").val(sreg[3]);
    }
...
function InsertNoConformidad() {
    if (ValidateFormAsignacionInconformidad()) {
        ...
        $('#btnGuardarInconformidad').attr('disabled', true);
        ...
        iInconformidad = JSON.stringify({ iIncoAspecto: iInconformidad, CodVisita: $("#txtCodigoVisita").val() });
                ...
                if (data != null) {
                    switch (data) {
                        case "success":
                            ...
                            $('#btnGuardarInconformidad').attr('disabled', false);
                            break;
                        default:
...
function DeleteRecordInconformidad(idIncoformidadRegistrada) {

    var params = new Object();
    params.IdInconformidadAspectos = idIncoformidadRegistrada;
